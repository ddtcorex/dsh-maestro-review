  ...
  function makeHandler(expectedPath: string) {
    return (req: IncomingMessage, res: ServerResponse) => {
      ...
      void requestAuthorized(req.headers['x-gitlab-token']).then(async (authorized) => {
        ...
        const pushEnabled = userConfig.autoRereviewOnPush === true
        ...
        const request = routeGitlabReviewRequest(body, botUsername, { pushEnabled })
        if (request === undefined) {
          const reason = describeDrop(body, botUsername, { pushEnabled })
          if (reason !== undefined) {
